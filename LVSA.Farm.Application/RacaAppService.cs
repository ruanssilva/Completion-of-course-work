...
{
    public class RacaAppService : AppServiceBase<RacaViewModel, Raca, IRacaService>, IRacaAppService
    {
        private IRetratoAppService _retratoAppService;

        public RacaAppService(IRacaService racaService, IRetratoAppService retratoAppService)
            : base(racaService)
        {
        ...
        public override IEnumerable<RacaViewModel> Todos()
        {
            return Filtrar(f => true);
        }

        public override IEnumerable<RacaViewModel> Filtrar(System.Linq.Expressions.Expression<Func<Raca, bool>> predicate)
        {
            var model = base.Filtrar(predicate);

            var retratos = _retratoAppService.Filtrar(f => f.RacaId != null).Select(s => s.Id).ToList();
        ...
        public override RacaViewModel Incluir(RacaViewModel model)
        {
        ...
        public override RacaViewModel Atualizar(RacaViewModel model)
        {
            ...
            var retirar = _retratoAppService.Filtrar(f => f.RacaId == model.Id && !inteiros.Contains(f.Id)).ToList();
