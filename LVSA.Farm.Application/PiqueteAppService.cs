...
{
    public class PiqueteAppService : AppServiceBase<PiqueteViewModel, Piquete, IPiqueteService>, IPiqueteAppService
    {
        private readonly IRetratoAppService _retratoAppService;
        
        public PiqueteAppService(IPiqueteService piqueteService,IRetratoAppService retratoAppService)
            : base(piqueteService)
        {
        ...
        public override IEnumerable<PiqueteViewModel> Todos()
        {
            return Filtrar(f => true);
        }
        
        public override IEnumerable<PiqueteViewModel> Filtrar(System.Linq.Expressions.Expression<Func<Piquete, bool>> predicate)
        {
            var model = base.Filtrar(predicate);

            var retratos = _retratoAppService.Filtrar(f => f.PiqueteId != null).Select(s => s.Id).ToList();
        ...
        public override PiqueteViewModel Incluir(PiqueteViewModel model)
        {
        ...
        public override PiqueteViewModel Atualizar(PiqueteViewModel model)
        {
            ...
            var retirar = _retratoAppService.Filtrar(f => f.PiqueteId == model.Id && !inteiros.Contains(f.Id)).ToList();
